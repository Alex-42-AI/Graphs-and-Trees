...
class DirectedGraph(Graph):
    ...
    def weighted_nodes_graph(self, weights: dict[Node, float] = None) \
        ...
        for n in self.nodes - set(weights):
            weights[n] = 0
        return WeightedNodesDirectedGraph({n: (weights[n], ([], self.next(n)))
                                           for n in self.nodes})

    def weighted_links_graph(self, weights: dict[tuple[Node, Node], float] = None) \
            -> "WeightedLinksDirectedGraph":
        if weights is None:
            weights = {l: 0 for l in self.links}
        for l in self.links - set(weights):
            weights[l] = 0
        return WeightedLinksDirectedGraph({u: ({}, {v: weights[(u, v)] for v in self.next(u)})
                                           for u in self.nodes})

    def weighted_graph(self, node_weights: dict[Node, float] = None, link_weights:
    dict[tuple[Node, Node], float] = None) -> "WeightedDirectedGraph":
    ...
    def subgraph(self, u_or_nodes: Node | Iterable[Node]) -> "DirectedGraph":
        try:
            ...
            return DirectedGraph({u: ([], self.next(u).intersection(u_or_nodes))
                                  for u in u_or_nodes})
        except TypeError:
    ...
    def euler_tour(self) -> list[Node]:
        if self.euler_tour_exists():
            ...
            return tmp.disconnect(u := (l := tmp.links.pop())[1],
                                  [v := l[0]]).euler_walk(u, v) + [u]
        return []
    ...
    def __str__(self) -> str:
        return ("<" + str(self.nodes) + ", {" +
                ", ".join(f"<{l[0]}, {l[1]}>" for l in self.links) + "}>")
...
class WeightedNodesDirectedGraph(DirectedGraph):
    ...
    dict[Node, tuple[float, tuple[Iterable[Node], Iterable[Node]]]] = {}) -> None:
        super().__init__()
        self.__node_weights = {}
    ...
    def copy(self) -> "WeightedNodesDirectedGraph":
        return WeightedNodesDirectedGraph({n: (self.node_weights(n),
                                               ([], self.next(n))) for n in self.nodes})

    def complementary(self) -> "WeightedNodesDirectedGraph":
        res = WeightedNodesDirectedGraph({n: (self.node_weights(n),
                                              ([], self.nodes)) for n in self.nodes})
        for l in self.links:
            res.disconnect(l[0], [l[1]])
        return res

    def transposed(self) -> "WeightedNodesDirectedGraph":
        return WeightedNodesDirectedGraph({u: (self.node_weights(u),
                                               (self.next(u), [])) for u in self.nodes})

    def weighted_graph(self, weights: dict[tuple[Node, Node], float] = None) \
    ...
    def undirected(self) -> "WeightedNodesUndirectedGraph":
        neighborhood = {n: (self.node_weights(n), self.prev(n).union(self.next(n)))
                        for n in self.nodes}
    ...
    def subgraph(self, u_or_nodes: Node | Iterable[Node]) -> "WeightedNodesDirectedGraph":
        try:
            ...
            if u_or_nodes not in self:
                raise KeyError("Unrecognized node!")
            queue, res = [u_or_nodes], WeightedNodesDirectedGraph(
            ...
            for possibility in product(*map(permutations, this_nodes_degrees)):
                ...
                for n, u in map_dict.items():
                    if self.node_weights(n) != other.node_weights(u):
                        ...
                        break
                    for m, v in map_dict.items():
                        if ((m in self.next(n)) ^ (v in other.next(u)) or self.node_weights(m)
                                != other.node_weights(v)):
                            possible = False
                            break
                    if not possible:
                        break
...
class WeightedLinksDirectedGraph(DirectedGraph):
    ...
    def __init__(self, neighborhood: dict[Node,
    tuple[dict[Node, float], dict[Node, float]]] = {}) -> None:
        super().__init__()
        self.__link_weights = {}
    ...
    def link_weights(self, u_or_l: Node | tuple = None, v: Node = None) \
            -> dict[Node, float] | dict[tuple[Node, Node], float] | float:
        """
        Args:
    ...
    def weighted_graph(self, weights: dict[Node, float] = None) -> "WeightedDirectedGraph":
        ...
        for n in self.nodes - set(weights):
            weights[n] = 0
        return WeightedDirectedGraph({u: (weights[u], ({}, {v: self.link_weights(u, v)
                                                            for v in self.next(u)}))
                                      for u in self.nodes})
    ...
    def subgraph(self, u_or_nodes: Node | Iterable[Node]) -> "WeightedLinksDirectedGraph":
        try:
            u_or_nodes = self.nodes.intersection(u_or_nodes)
            neighborhood = {u: ({}, {k: v for k, v in self.link_weights(u).items()
                                     if k in u_or_nodes}) for u in u_or_nodes}
    ...
    def __str__(self) -> str:
        return ("<" + str(self.nodes) + ", {" +
                ", ".join(f"<{l[0]}, {l[1]}> -> {self.link_weights(l)}"
                          for l in self.links) + "}>")
...
class WeightedDirectedGraph(WeightedLinksDirectedGraph, WeightedNodesDirectedGraph):
    ...
    def __init__(self, neighborhood: dict[Node,
    tuple[float, tuple[dict[Node, float], dict[Node, float]]]] = {}) -> None:
    ...
    def subgraph(self, u_or_nodes: Node | Iterable[Node]) -> "WeightedDirectedGraph":
        try:
            ...
            neighborhood = {u: (self.node_weights(u),
                                ({}, {k: v for k, v in self.link_weights(u).items()
                                      if k in u_or_nodes})) for u in u_or_nodes}
            return WeightedDirectedGraph(neighborhood)
        except TypeError:
            if not isinstance(u_or_nodes, Node):
            ...
            def dijkstra(s, curr_path, curr_weight):
                ...
                for n in curr_tmp.nodes:
                    ...
                    for m in curr_tmp.next(n):
                        weights_from_to[n][m] = (curr_tmp.link_weights(n, m) +
                                                 curr_tmp.node_weights(m))
                        paths[n][m] = [(n, m)]
        ...
        if isinstance(other, WeightedDirectedGraph):
            ...
            for possibility in product(*map(permutations, this_nodes_degrees)):
                ...
                for n, u in map_dict.items():
                    ...
                    for m, v in map_dict.items():
                        if (self.link_weights(n, m) != other.link_weights(u, v)
                                or self.node_weights(m) != other.node_weights(v)):
                            possible = False
                            break
                    if not possible:
                        break
    ...
    def __str__(self) -> str:
        return ("<{" + ", ".join(f"{n} -> {self.node_weights(n)}" for n in self.nodes) +
                ", " + ", ".join(f"<{l[0]}, {l[1]}> -> {self.link_weights(l)}" for l in self.links)
                + "}>")
