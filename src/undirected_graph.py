...
class UndirectedGraph(Graph):
    ...
    def is_tree(self, connected: bool = False) -> bool:
        """
        Args:
            connected: Boolean flag indicating whether the graph is already known to be connected
        Returns:
...
class WeightedUndirectedGraph(WeightedLinksUndirectedGraph, WeightedNodesUndirectedGraph):
    ...
    def a_star(self, u: Node, v: Node, h: Callable) -> Path:
        ...
        if any(w < 0 for w in self.node_weights()) or any(w < 0 for w in self.link_weights()) or v not in (subgraph := self.component(u)):
            return []
        ...
        pq = [(h(u), u)]
        g = {n: inf for n in subgraph.nodes}
        g[u] = subgraph.node_weights(u)
        parent = {n: None for n in subgraph.nodes}
        ...
        while pq:
            ...
            for y in subgraph.neighbors(x):
                if (g_y := g[x] + subgraph.link_weights(x, y) + subgraph.node_weights(y)) < g[y]:
                    parent[y], g[y] = x, g_y
    ...
    def ida_star(self, u: Node, v: Node, h: Callable) -> Path:
        def dfs(x, g):
            ...
            next_nodes = sorted(subgraph.neighbors(x) - so_far, key=lambda n: h(n) + subgraph.link_weights(x, n) + subgraph.node_weights(n))
            f = inf

            for y in next_nodes:
                path.append(y), so_far.add(y)
                t = dfs(y, g + subgraph.link_weights(x, y) + subgraph.node_weights(y))
