...
from .base import Node, Graph, Iterable
...
def sat_to_clique(cnf: list[list[tuple[str, bool]]]) -> list[set[tuple[str, bool]]]:
    ...
    # First simplify cnf. If any disjunctive clause is a tautology, it's removed.
    i = 0
    while i < len(cnf):
...
class UndirectedGraph(Graph):
    ...
    def tree(self, n: Node = None, depth: bool = False) -> "Tree":
        ...
        rest, total = [n], {n}
        while rest:
            for v in self.neighbors(u := rest.pop(-bool(depth))) - total:
    ...
    def links_graph(self) -> "UndirectedGraph":
        ...
        """
        neighborhood = {Node(l0): [Node(l1) for l1 in self.links if (l1.u in l0) or (l1.v in l0)]
                        for l0 in self.links}
        return UndirectedGraph(neighborhood)

    def weighted_nodes_graph(self, weights: dict[Node, float] = None) \
            -> "WeightedNodesUndirectedGraph":
        if weights is None:
            weights = {n: 0 for n in self.nodes}
        for n in self.nodes - set(weights):
            weights[n] = 0
        return WeightedNodesUndirectedGraph({n: (weights[n], self.neighbors(n))
                                             for n in self.nodes})

    def weighted_links_graph(self, weights: dict[Link, float] = None) \
            -> "WeightedLinksUndirectedGraph":
        if weights is None:
            weights = {l: 0 for l in self.links}
        for l in self.links - set(weights):
            weights[l] = 0
        return WeightedLinksUndirectedGraph({u: {v: weights[Link(u, v)]
                                                 for v in self.neighbors(u)} for u in self.nodes})
    ...
    def interval_sort(self, start: Node = None) -> list[Node]:
        """
        Assume a set of intervals on the real numberline, some of which could intersect. Such a set
        of intervals can be sorted based on multiple criteria. An undirected graph could be defined
        to represent the intervals the following way: Nodes represent the intervals and two nodes
        are connected exactly when the intervals they represent intersect.
        Args:
        ...
        def helper(u, graph, priority):
            ...
            for comp in comps:
                ...
                starting = bfs(curr_graph := graph.subgraph(comp), comp[-1],
    ...
    def is_full_k_partite(self, k: int = None) -> bool:
        ...
        return (k in {None, len(comps := self.complementary().connection_components())}
                and all(c.full() for c in comps))
    ...
    def chromatic_links_partition(self) -> list[set[Link]]:
        ...
        return [set(map(lambda x: x.value, s)) for s in
                UndirectedGraph.links_graph(self).chromatic_nodes_partition()]
    ...
    def hamilton_tour_exists(self) -> bool:
        ...
        if (n := len(self.nodes)) == 1 or (2 * (m := len(self.links)) > (n - 1) * (n - 2) + 2 or
                                           n > 2 and all(
    ...
    def __add__(self, other: "UndirectedGraph") -> "UndirectedGraph":
        ...
        if not isinstance(other, UndirectedGraph):
            raise TypeError(f"Addition not defined between "
                            f"class UndirectedGraph and type {type(other).__name__}!")
...
class WeightedNodesUndirectedGraph(UndirectedGraph):
    ...
    def copy(self) -> "WeightedNodesUndirectedGraph":
        return WeightedNodesUndirectedGraph({n: (self.node_weights(n), self.neighbors(n))
                                             for n in self.nodes})

    def complementary(self) -> "WeightedNodesUndirectedGraph":
        res = WeightedNodesUndirectedGraph({u: (self.node_weights(u), self.nodes)
                                            for u in self.nodes})
        for l in self.links:
            res.disconnect(l.u, l.v)
        return res

    def weighted_tree(self, n: Node = None, depth: bool = False) -> "WeightedTree":
        """
        Args:
            n: A present node.
            depth: a boolean flag, answering to whether the search algorithm should use DFS or BFS.
        ...
        queue, total = [n], {n}
        while queue:
            for v in self.neighbors(u := queue.pop(-bool(depth))) - total:
    ...
    def subgraph(self, nodes: Iterable[Node]) -> "WeightedNodesUndirectedGraph":
        try:
            nodes = self.nodes.intersection(nodes)
            neighborhood = {u: (self.node_weights(u), self.neighbors(u).intersection(nodes))
                            for u in nodes}
        ...
        for l0 in self.links:
            for l1 in self.links:
                if l0 != l1 and (s := {l0.u, l0.v}.intersection({l1.u, l1.v})):
                    result.connect(Node(l0),
                                   {Node(l1): self.node_weights(Node(s.pop()))})
        return result
    ...
    def weighted_dominating_set(self) -> set[Node]:
        ...
        if not self:
            return set()
        if not self.connected():
            return reduce(lambda x, y: x.union(y), [comp.weighted_dominating_set()
            ...
            for possibility in product(*map(permutations, this_nodes_degrees)):
                ...
                for n, u in map_dict.items():
                    if self.node_weights(n) != other.node_weights(u):
                        ...
                        break
                    for m, v in map_dict.items():
                        if ((m in self.neighbors(n)) ^ (v in other.neighbors(u)) or
                                self.node_weights(m) != other.node_weights(v)):
                            possible = False
                            break
                    if not possible:
                        break
    ...
    def __add__(self, other: UndirectedGraph) -> "WeightedNodesUndirectedGraph":
        if not isinstance(other, UndirectedGraph):
            raise TypeError(f"Addition not defined between class "
                            f"WeightedNodesUndirectedGraph and type {type(other).__name__}!")
    ...
    def __str__(self) -> str:
        return ("<{" + ", ".join(f"{n} -> {self.node_weights(n)}" for n in self.nodes) + "}, " +
                str(self.links) + ">")
...
class WeightedLinksUndirectedGraph(UndirectedGraph):
    ...
    def link_weights(self, u_or_l: Node | Link = None, v: Node = None) \
            -> dict[Node, float] | dict[Link, float] | float:
        """
        Args:
    ...
    def subgraph(self, nodes: Iterable[Node]) -> "WeightedLinksUndirectedGraph":
        try:
            nodes = self.nodes.intersection(nodes)
            neighborhood = {u: {k: v for k, v in self.link_weights(u).items() if k in nodes}
                            for u in nodes}
    ...
    def links_graph(self) -> WeightedNodesUndirectedGraph:
        neighborhood = {Node(l0): (self.link_weights(l0), [Node(l1) for l1 in self.links
                                                           if (l1.u in l0) ^ (l1.v in l0)]) for l0
                        in self.links}
        return WeightedNodesUndirectedGraph(neighborhood)
    ...
    def __add__(self, other: UndirectedGraph) -> "WeightedLinksUndirectedGraph":
        if not isinstance(other, UndirectedGraph):
            raise TypeError(f"Addition not defined between class "
                            f"WeightedLinksUndirectedGraph and type {type(other).__name__}!")
        if isinstance(other, WeightedNodesUndirectedGraph):
            return other + self
        if isinstance(other, WeightedLinksUndirectedGraph):
            res = self.copy()
    ...
    def __str__(self) -> str:
        return ("<" + str(self.nodes) + ", {" +
                ", ".join(f"{l} -> {self.link_weights(l)}" for l in self.links) + "}>")
...
class WeightedUndirectedGraph(WeightedLinksUndirectedGraph, WeightedNodesUndirectedGraph):
    ...
    def subgraph(self, nodes: Iterable[Node]) -> "WeightedUndirectedGraph":
        try:
            nodes = self.nodes.intersection(nodes)
            neighborhood = {u: (self.node_weights(u),
    ...
    def minimal_path(self, u: Node, v: Node) -> list[Node]:
        ...
        def dfs(x, current_path, current_weight, total_negative, res_path=None, res_weight=0):
            def dijkstra(s, curr_path, curr_weight):
                ...
                weights_from_to = {n: {m: curr_tmp.total_weight for m in curr_tmp.nodes}
                                   for n in curr_tmp.nodes}
                for n in curr_tmp.nodes:
                    weights_from_to[n][n] = 0
                    for m in curr_tmp.neighbors(n):
                        weights_from_to[n][m] = (
                                curr_tmp.link_weights(n, m) + curr_tmp.node_weights(m))
                        paths[n][m] = [Link(n, m)]
        ...
        if v in self:
            if v in (tmp := self.component(u)):
                nodes_negative_weights = sum(tmp.node_weights(n)
                                             for n in tmp.nodes if tmp.node_weights(n) < 0)
                links_negative_weights = sum(tmp.link_weights(l)
                                             for l in tmp.links if tmp.link_weights(l) < 0)
                res = dfs(u, [],
                          tmp.node_weights(u), nodes_negative_weights + links_negative_weights)
            ...
            for possibility in product(*map(permutations, this_nodes_degrees)):
                ...
                for n, u in map_dict.items():
                    if self.node_weights(n) != other.node_weights(u):
                        ...
                        break
                    for m, v in map_dict.items():
                        if (self.link_weights(Link(n, m)) != other.link_weights(Link(u, v)) or
                                self.node_weights(m) != other.node_weights(v)):
                            possible = False
                            break
                    if not possible:
    ...
    def __add__(self, other: UndirectedGraph) -> "WeightedUndirectedGraph":
        if not isinstance(other, UndirectedGraph):
            raise TypeError(f"Addition not defined between class "
                            f"WeightedUndirectedGraph and type {type(other).__name__}!")
        if isinstance(other, WeightedUndirectedGraph):
            res = self.copy()
            for n in other.nodes:
                if n in res:
