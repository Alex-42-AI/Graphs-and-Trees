...
class UndirectedGraph:
    ...
    def neighboring(self, u: Node = None):
        if u is None: return self.__neighboring
        if isinstance(u, Node):
            if u in self.nodes(): return self.neighboring(u)
    ...
    def degrees(self, u: Node = None):
        ...
        if isinstance(u, Node):
            if u in self.__nodes: return self.__degrees[u]
            raise KeyError('No such node in the graph!')
        raise TypeError('Node expected!')
    ...
    def add(self, u: Node, *current_nodes: Node):
        if u not in self.nodes():
            ...
            for c in current_nodes:
                if c in self.nodes() and c not in res: res.append(c)
    ...
    def connect(self, u: Node, v: Node, *rest: Node):
        for n in [v] + [*rest]:
            if Link(u, n) not in self.links() and u != n and n in self.nodes():
                self.__degrees[u] += 1
                self.__degrees[n] += 1
    ...
    def complementary(self):
        res = UndirectedGraph(*self.nodes())
    ...
    def copy(self):
        res = UndirectedGraph(*self.nodes())
        for n in self.nodes():
            if self.degrees(n): res.connect(n, *self.neighboring(n))
        return res
    ...
    def __connected(nodes: [Node], links: [Link]):
        if len(links) < len(nodes) - 1: return False
        if 2 * len(links) > (len(nodes) - 1) * (len(nodes) - 2) or len(nodes) == 1: return True
        ...
        return False
    def connected(self):
        return self.__connected(self.nodes(), self.links())
    def tree(self):
    ...
    def full(self):
        return self.__full(self.nodes(), self.links())
    ...
    def cliques(self, k: int):
        ...
        if k == 1: return list(map(list, self.nodes()))
        if k == len(self.nodes()): return [[], [self.nodes()]][self.full()]
        result = SortedList()
        for p in permutations(self.nodes(), k):
            can = True
    ...
    def chromaticNumberNodes(self):
        def helper(nodes: [Node] = None, curr=0, so_far: SortedList = None, _except: [Node] = None):
            if len(self.links()) > 30:
                nodes = sorted(self.nodes(), key=lambda _n: self.degrees(_n))
                ...
                while k < n and queue:
                    ...
                    for v in filter(lambda x: x not in total, self.neighboring(u)):
                        total.insert(v), queue.append(v)
                        cols, k = [colors[_n] for _n in filter(lambda x: x in total, self.neighboring(v))], k + 1
            ...
            if _except is None: _except = []
            if nodes is None: nodes = sorted(self.nodes(), key=lambda _n: self.degrees(_n))
            ...
            res = len(self.nodes())
            for u in rest:
                _res = helper([u] + [_n for _n in nodes if _n not in (nodes[0], u)], curr, so_far, _except + [_n for _n in nodes if Link(nodes[0], _n) in self.links() or Link(u, _n) in self.links()])
...
class WeightedNodesUndirectedGraph(UndirectedGraph):
    ...
    def copy(self):
        res = WeightedNodesUndirectedGraph(*[(n, self.node_weights(n)) for n in self.nodes()])
        for n in self.nodes():
            if self.degrees(n): res.connect(n, *self.neighboring(n))
        return res
...
class WeightedLinksUndirectedGraph(UndirectedGraph):
    ...
    def link_weights(self, u_or_l: Node | Link = None, v: Node = None):
        if u_or_l is None: return self.__link_weights
            ...
            if v is None: return SortedKeysDict(*[(n, self.__link_weights[Link(n, u_or_l)]) for n in self.neighboring(u_or_l)])
            if isinstance(v, Node):
                if v in self.nodes():
                    if Link(u_or_l, v) in self.links(): return self.__link_weights[Link(u_or_l, v)]
    ...
    def add(self, u: Node, *nodes_weights: (Node, float)):
        if u not in self.nodes():
            ...
            for v, w in nodes_weights:
                if v in self.nodes() and v not in [p[0] for p in res]: res.append((v, w))
            for v, w in res:
                if not isinstance(w, (int, float)): raise TypeError('Real numerical values expected!')
            super().add(u, *[p[0] for p in res])
            for v, w in res: self.__link_weights[Link(u, v)] = w
    def remove(self, n: Node, *rest: Node):
        for u in (n,) + rest:
    ...
    def connect(self, u: Node, v_w: (Node, float), *nodes_weights: (Node, float)):
        if u in self.nodes():
            ...
            for v, w in [v_w] + list(nodes_weights):
                if Link(u, v) not in self.link_weights(): self.__link_weights[Link(u, v)] = w
    def disconnect(self, u: Node, v: Node, *rest: Node):
    ...
    def copy(self):
        ...
        for u in self.nodes():
            for v in self.nodes():
                if Link(u, v) in self.links(): res.connect(u, (v, self.link_weights(u, v)))
        return res
    def euler_tour(self):
        if self.euler_tour_exists():
    ...
    def minimal_spanning_tree(self):
        if self.tree(): return self.links(), self.total_weight()
        if not self.connected():
            res = []
            for comp in self.connection_components():
                curr = WeightedLinksUndirectedGraph(*comp)
                for u in comp:
                    for v in self.neighboring(u): curr.connect(u, (v, self.link_weights(u, v)))
    ...
    def isomorphic(self, other):
        if isinstance(other,WeightedLinksUndirectedGraph):
            ...
            for possibility in possibilities:
                ...
                for n, u in map_dict.items():
                    for m, v in map_dict.items():
                        if self.link_weights(Link(u, v)) != other.link_weights(Link(u, v)):
                            possible = False
                            break
                    if not possible: break
                if possible: return True
