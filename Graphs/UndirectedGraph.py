...
class UndirectedGraph(Graph):
    ...
    def interval_sort(self) -> list[Node]::
        ...
        def lex_bfs(graph, u):
            labels, order = {node: int(node == u) for node in graph.nodes}, []
            order.append(u), labels.pop(u)
            while labels:
    ...
    def cliquesGraph(self) -> "UndirectedGraph":
...
class WeightedNodesUndirectedGraph(UndirectedGraph):
    ...
    def component(self, n: Node) -> "WeightedNodesUndirectedGraph":
        if n not in self:
            raise ValueError("Unrecognized node!")
...
class WeightedUndirectedGraph(WeightedNodesUndirectedGraph, WeightedLinksUndirectedGraph):
    ...
    def component(self, n: Node) -> "WeightedUndirectedGraph":
        if n not in self:
            raise ValueError("Unrecognized node!")
