...
class UndirectedGraph:
    def __init__(self, neighborhood: Dict = Dict(), f=lambda x: x):
        ...
        for u, neighbors in neighborhood.items():
            if u not in self.nodes(): self.add(u)
            for v in neighbors:
                if v in self.nodes(): self.connect(u, v)
                else: self.add(v, u)
    def nodes(self): return self.__nodes
    def links(self): return self.__links
    def neighboring(self, u: Node = None): return self.__neighboring if u is None else self.__neighboring[u]
    def degrees(self, u: Node = None): return self.__degrees if u is None else self.__degrees[u]
    def degrees_sum(self): return 2 * len(self.links())
    def f(self, x=None): return self.__f if x is None else self.__f(x)
    def add(self, u: Node, *current_nodes: Node):
        if u not in self.nodes():
            self.__nodes.insert(u)
            self.__degrees[u], self.__neighboring[u] = 0, SortedList(self.f())
            if current_nodes: UndirectedGraph.connect(self, u, *current_nodes)
    ...
    def copy(self): return UndirectedGraph(self.neighboring(), self.f())
    def width(self):
        res = 0
        for u in self.nodes():
            ...
            while queue:
                ...
                while queue:
                    u = queue.pop(0)
                    for v in filter(lambda x: x not in total, self.neighboring(u)): total.insert(v), new.append(v)
                queue = new.copy()
                _res += bool(new)
    ...
    def component(self, u: Node):
        if u not in self.nodes(): raise ValueError("Unrecognized node!")
        queue, res = [u], UndirectedGraph(Dict((u, [])), self.f())
        while queue:
            v = queue.pop(0)
            for n in self.neighboring(v):
                if n in res.nodes(): res.connect(v, n)
    ...
    def bridge_links(self):
        ...
        for n in self.nodes():
            if not colors[n]: dfs(n, 0)
        return res
    def full(self): return self.degrees_sum() == len(self.nodes()) * (len(self.nodes()) - 1)
    ...
    def euler_walk_exists(self, u: Node, v: Node):
        if u not in self.nodes() or v not in self.nodes(): raise ValueError("Unrecognized node(s)!")
        if u == v: return self.euler_tour_exists()
        for n in self.nodes():
    ...
    def euler_tour(self):
        if self.euler_tour_exists():
            ...
            tmp.disconnect(u, v)
            return tmp.euler_walk(u, v) + [Link(v, u)]
        return False
            ...
            for y in self.neighboring(x):
                if Link(x, y) not in result + stack:
                    if y == n:
                        ...
                        while stack: result.insert(i + 1, stack.pop())
                        return True
                    if dfs(y, stack + [Link(x, y)]): return True
        if u not in self.nodes() or v not in self.nodes(): raise Exception('Unrecognized node(s)!')
    ...
    def clique(self, n: Node, *nodes: Node):
        ...
        if any(u not in self.neighboring(n) for u in res): return False
    ...
    def cliques(self, k: int):
        ...
        for p in combinations(self.nodes(), abs(k)):
            if self.clique(*p): result.append(list(p))
        return result
    def chromaticNumberNodes(self):
        def helper(curr):
            if not tmp.nodes(): return curr
            if tmp.full(): return curr + list(map(lambda x: [x], tmp.nodes()))
            _result = max_nodes
            for anti_clique in tmp.independentSet():
                ...
                if len(res) < len(_result): _result = res
            return _result
        if not self.connected():
            r = []
            for c in self.connection_components(): r.append(self.component(c[0]).chromaticNumberNodes())
            final = r[0]
            for c in r[1:]:
                for i in range(min(len(c), len(final))): final[i] += c[i]
                if len(c) > len(final):
                    for i in range(len(final), len(c)): final.append(c[i])
            return final
        if self.tree():
            if not self: return [[]]
    ...
    def chromaticNumberLinks(self):
        ...
        for u in res_graph.nodes():
            for v in res_graph.nodes():
                if v != u and (u.value().u() in v.value() or u.value().v() in v.value()): res_graph.connect(u, v)
    ...
    def loopWithLength(self, length: int):
        ...
        for l in tmp.links():
            u, v = l.u(), l.v()
            tmp.disconnect(u, v)
            res = tmp.pathWithLength(v, u, abs(length) - 1)
            tmp.connect(u, v)
            if res: return [l] + res
        return False
    def vertexCover(self):
        ...
        def helper(curr, i=0):
            if not tmp.links(): return [curr.copy()]
            result = [nodes]
            for j in range(i, len(nodes)):
                u = nodes[j]
                neighbors = tmp.neighboring(u).copy()
                if neighbors:
                    tmp.remove(u), curr.insert(u)
                    res = helper(curr, j + 1)
                    tmp.add(u, *neighbors), curr.remove(u)
                    if len(res[0]) == len(result[0]): result += res
                    elif len(res[0]) < len(result[0]): result = res
            return result
    ...
    def dominatingSet(self):
        ...
        def helper(curr, total, i=0):
            ...
            for j in range(i, len(nodes)):
                ...
                for v in self.neighboring(u):
                    if v not in total: new.insert(v)
        ...
        for n in nodes:
            if not self.degrees(n): isolated.insert(n), nodes.remove(n)
        return helper(isolated, isolated)
    def independentSet(self):
        result = []
        for res in self.vertexCover(): result.append(list(filter(lambda x: x not in res, self.nodes())))
        return result
    def hamiltonTourExists(self):
        def dfs(x):
            if tmp.nodes() == [x]: return x in can_end_in
            if all(y not in tmp.nodes() for y in can_end_in): return False
        ...
        if any(self.degrees(n) <= 1 for n in self.nodes()) or not self.connected(): return False
        if all(2 * self.degrees(n) >= len(self.nodes()) for n in self.nodes()) or 2 * len(self.links()) > (len(self.nodes()) - 1) * (len(self.nodes()) - 2) + 2: return True
        u, tmp = self.nodes()[0], UndirectedGraph.copy(self)
        can_end_in = tmp.neighboring(u).copy()
        return dfs(u)
    def hamiltonWalkExists(self, u: Node, v: Node):
        if u not in self.nodes() or v not in self.nodes(): raise Exception("Unrecognized node(s).")
        if v in self.neighboring(u): return True if all(n in (u, v) for n in self.nodes()) else self.hamiltonTourExists()
        tmp = UndirectedGraph.copy(self)
        tmp.connect(u, v)
        return tmp.hamiltonTourExists()
    def hamiltonTour(self):
        if any(self.degrees(n) < 2 for n in self.nodes()) or not self.connected(): return False
        u = self.nodes()[0]
        for v in self.neighboring(u):
    ...
    def hamiltonWalk(self, u: Node = None, v: Node = None):
        def dfs(x, stack):
            if not tmp.degrees(x) or v is not None and not tmp.degrees(v): return False
            too_many = v is not None
            for n in tmp.nodes():
                if n not in (x, v) and tmp.degrees(n) < 2:
                    if too_many: return False
                    too_many = True
            if not tmp.nodes(): return stack
            neighbors = tmp.neighboring(x).copy()
            tmp.remove(x)
        ...
        tmp = UndirectedGraph.copy(self)
        if u is None:
            if v is not None and v not in self.nodes(): raise Exception("Unrecognized node.")
            for _u in sorted(self.nodes(), key=lambda _x: self.degrees(_x)):
                result = dfs(_u, [_u])
                if result: return result
                if self.degrees(_u) == 1: return False
            return False
        if u not in self.nodes() or v is not None and v not in self.nodes(): raise Exception("Unrecognized node(s).")
        return dfs(u, [u])
    ...
    def __reversed__(self): return self.complementary()
    def __contains__(self, item): return item in self.nodes() or item in self.links()
    def __add__(self, other):
        if not isinstance(other, UndirectedGraph): raise TypeError(f"Addition not defined between class UndirectedGraph and type {type(other).__name__}!")
        if any(self(x) != other(x) for x in self.nodes() + other.nodes()): raise ValueError("Node sorting functions don't match!")
        if isinstance(other, (WeightedNodesUndirectedGraph, WeightedLinksUndirectedGraph)): return other + self
        res = self.copy()
        for n in other.nodes():
            if n not in res.nodes(): res.add(n)
        for l in other.links():
            if l not in res.links(): res.connect(l.u(), l.v())
        return res
    def __eq__(self, other):
        if isinstance(other, UndirectedGraph):
            ...
            return True
        return False
    def __str__(self): return '({' + ', '.join(str(n) for n in self.nodes()) + '}, {' + ', '.join(str(l) for l in self.links()) + '})'
    def __repr__(self): return str(self)
class WeightedNodesUndirectedGraph(UndirectedGraph):
    def __init__(self, neighborhood: Dict = Dict(), f=lambda x: x):
        super().__init__(Dict(), f)
        self.__node_weights = SortedKeysDict(f=f)
        for n, p in neighborhood.items(): self.add((n, p[0]))
        for u, p in neighborhood.items():
            for v in p[1]:
                if v in self.nodes(): self.connect(u, v)
                else: self.add((v, 0), u)
    def node_weights(self, u: Node = None): return self.__node_weights[u] if u is not None else self.__node_weights
    ...
    def set_weight(self, u: Node, w: float):
        if u in self.nodes(): self.__node_weights[u] = w
    def copy(self): return WeightedNodesUndirectedGraph(Dict(*[(n, (self.node_weights(n), self.neighboring(n))) for n in self.nodes()]), self.f())
    def component(self, u: Node):
        if u not in self.nodes(): raise ValueError("Unrecognized node!")
    ...
    def weightedVertexCover(self):
        ...
        def helper(curr, i=0, res_sum=0):
            if not tmp.links(): return [curr.copy()], res_sum
            ...
            for j in range(i, len(nodes)):
                ...
                tmp.add((u, w), *neighbors), curr.remove(u)
                if res[1] == result_sum: result += res[0]
                elif res[1] < result_sum: result, result_sum = res
    ...
    def weightedDominatingSet(self):
        ...
        def helper(curr, total, i=0):
            ...
            for j in range(i, len(nodes)):
                ...
                if res_sum == result_sum: result += res
                elif res_sum < result_sum: result, result_sum = res, res_sum
                curr.remove(u)
            return result
        isolated = SortedList(self.f())
        for n in nodes:
    ...
    def __add__(self, other):
        if not isinstance(other, UndirectedGraph): raise TypeError(f"Addition not defined between class WeightedNOdesUndirectedGraph and type {type(other).__name__}!")
        if any(self(x) != other(x) for x in self.nodes() + other.nodes()): raise ValueError("Node sorting functions don't match!")
        ...
        if isinstance(other, WeightedNodesUndirectedGraph):
            ...
            for l in other.links(): res.connect(l.u(), l.v())
            return res
        if isinstance(other, WeightedLinksUndirectedGraph): return WeightedUndirectedGraph(self.f()) + self + other
        for n in other.nodes():
    ...
    def __eq__(self, other):
        if isinstance(other, WeightedNodesUndirectedGraph):
            ...
            return True
        return False
    def __str__(self): return '({' + ', '.join(f'{str(n)} -> {self.node_weights(n)}' for n in self.nodes()) + '}, {' + ', '.join(str(l) for l in self.links()) + '})'
class WeightedLinksUndirectedGraph(UndirectedGraph):
    def __init__(self, neighborhood: Dict = Dict(), f=lambda x: x):
        ...
        for u, neighbors in neighborhood.items():
            ...
            for v, w in neighbors.items():
                if v not in self.nodes(): self.add(v, Dict((u, w)))
                elif v not in self.neighboring(u): self.connect(u, Dict((v, w)))
    def link_weights(self, u_or_l: Node | Link = None, v: Node = None):
        if u_or_l is None: return self.__link_weights
            ...
            if v is None: return SortedKeysDict(*[(n, self.__link_weights[Link(n, u_or_l)]) for n in self.neighboring(u_or_l)], f=self.f())
            return self.__link_weights[Link(u_or_l, v)]
        elif isinstance(u_or_l, Link): return self.__link_weights[u_or_l]
    ...
    def add(self, u: Node, nodes_weights: Dict = Dict()):
        if u not in self.nodes():
            ...
            for v, w in nodes_weights.items(): self.__link_weights[Link(u, v)] = w
    def remove(self, n: Node, *rest: Node):
        for u in (n,) + rest:
            for v in self.neighboring(u): self.__link_weights.pop(Link(u, v))
        super().remove(n, *rest)
    def connect(self, u: Node, nodes_weights: Dict = Dict()):
        if u in self.nodes():
            ...
            if nodes_weights: super().connect(u, *nodes_weights.keys())
    def disconnect(self, u: Node, v: Node, *rest: Node):
    ...
    def set_weight(self, l: Link, w: float):
        if l in self.links(): self.__link_weights[l] = w
    def copy(self): return WeightedLinksUndirectedGraph(Dict(*[(n, self.link_weights(n)) for n in self.nodes()]), self.f())
    def component(self, u: Node):
        if u not in self.nodes(): raise ValueError("Unrecognized node!")
        ...
        while queue:
            v = queue.pop(0)
            for n in self.neighboring(v):
                if n in res.nodes(): res.connect(v, Dict((n, self.link_weights(v, n))))
                else: res.add(n, Dict((v, self.link_weights(v, n)))), queue.append(n)
        return res
    def minimal_spanning_tree(self):
        ...
        if not self.connected():
            ...
            return res
        if not self.links(): return [], 0
        res_links, total, bridge_links = [], SortedList(self.f()), SortedList(lambda x: self.link_weights(x))
        total.insert(self.nodes()[0])
        for u in self.neighboring(self.nodes()[0]): bridge_links.insert(Link(self.nodes()[0], u))
        for _ in range(1, len(self.nodes())):
            ...
            if u in total:
                ...
                for _v in self.neighboring(v):
                    if _v in total: bridge_links.remove(Link(v, _v))
                    else: bridge_links.insert(Link(v, _v))
            else:
                total.insert(u)
                for _u in self.neighboring(u):
                    if _u in total: bridge_links.remove(Link(u, _u))
                    else: bridge_links.insert(Link(u, _u))
    ...
    def minimalPathLinks(self, u: Node, v: Node):
        def dfs(x, curr_path, curr_w, total_negative, res_path=None, res_w=0):
            ...
            for y in filter(lambda _y: Link(x, _y) not in curr_path, self.neighboring(x)):
                ...
                if y == v and (curr_w + self.link_weights(Link(x, y)) < res_w or not res_path): res_path, res_w = curr_path + [Link(x, y)], curr_w + self.link_weights(Link(x, y))
    ...
    def isomorphic(self, other: UndirectedGraph):
        ...
        return super().isomorphic(other)
    def __add__(self, other):
        if not isinstance(other, UndirectedGraph): raise TypeError(f"Addition not defined between class WeightedLinksUndirectedGraph and type {type(other).__name__}!")
        if any(self(x) != other(x) for x in self.nodes() + other.nodes()): raise ValueError("Node sorting functions don't match!")
        ...
        if isinstance(other, WeightedLinksUndirectedGraph):
            for n in other.nodes():
                if n not in res.nodes(): res.add(n)
            for l in other.links():
                if l in res.links(): res.set_weight(l, res.link_weights(l) + other.link_weights(l))
                else: res.connect(l.u(), Dict((l.v(), other.link_weights(l))))
            return res
        for n in other.nodes():
            if n not in res.nodes(): res.add(n)
        for l in other.links():
            if l not in res.links(): res.connect(l.u(), Dict((l.v(), 0)))
        return res
    def __eq__(self, other):
        if isinstance(other, WeightedLinksUndirectedGraph): return self.nodes() == other.nodes() and self.link_weights() == other.link_weights()
        return False
    def __str__(self): return '({' + ', '.join(str(n) for n in self.nodes()) + '}, ' + str(self.link_weights()) + ')'
class WeightedUndirectedGraph(WeightedNodesUndirectedGraph, WeightedLinksUndirectedGraph):
    def __init__(self, neighborhood: Dict = Dict(), f=lambda x: x):
        ...
        for u, p in neighborhood.items():
            for v, w in p[1].items():
                if v in self.nodes(): self.connect(u, Dict((v, w)))
                else: self.add((v, 0), Dict((u, w)))
    ...
    def remove(self, u: Node, *rest: Node):
        for n in (u,) + rest:
            tmp = self.neighboring(u)
            if tmp: WeightedLinksUndirectedGraph.disconnect(self, u, *tmp)
            super().remove(n)
    def connect(self, u: Node, nodes_weights: Dict = Dict()): WeightedLinksUndirectedGraph.connect(self, u, nodes_weights)
    def disconnect(self, u: Node, v: Node, *rest: Node): WeightedLinksUndirectedGraph.disconnect(self, u, v, *rest)
    def set_weight(self, el: Node | Link, w: float):
        if el in self.nodes(): WeightedNodesUndirectedGraph.set_weight(self, el, w)
        elif el in self.links(): WeightedLinksUndirectedGraph.set_weight(self, el, w)
    def copy(self): return WeightedUndirectedGraph(Dict(*[(n, (self.node_weights(n), self.link_weights(n))) for n in self.nodes()]), self.f())
    def component(self, u: Node):
        if u not in self.nodes(): raise ValueError("Unrecognized node!")
        ...
        while queue:
            ...
            for n in self.neighboring(v):
                if n in res.nodes(): res.connect(v, Dict((n, self.link_weights(v, n))))
                else: res.add((n, self.node_weights(n)), Dict((v, self.link_weights(v, n)))), queue.append(n)
        return res
    ...
    def isomorphic(self, other: UndirectedGraph):
        if isinstance(other, WeightedUndirectedGraph):
            ...
            for possibility in possibilities:
                ...
                if possible: return True
            return False
        if isinstance(other, (WeightedNodesUndirectedGraph, WeightedLinksUndirectedGraph)): return type(other).isomorphic(other, self)
        return UndirectedGraph.isomorphic(self, other)
    def __add__(self, other):
        if not isinstance(other, UndirectedGraph): raise TypeError(f"Addition not defined between class WeightedUndirectedGraph and type {type(other).__name__}!")
        if any(self(x) != other(x) for x in self.nodes() + other.nodes()): raise ValueError("Node sorting functions don't match!")
        res = self.copy()
        if isinstance(other, WeightedUndirectedGraph):
            for n in other.nodes():
            ...
            for u in other.nodes():
                for v in other.neighboring(u):
                    if v in res.neighboring(u): res.set_weight(Link(u, v), res.link_weights(u, v) + other.link_weights(u, v))
                    else: res.connect(u, Dict((v, other.link_weights(u, v))))
            ...
            for u in other.nodes():
                for v in other.neighboring(u):
                    if v not in res.neighboring(u): res.connect(u, Dict((v, 0)))
        ...
        return res
    def __eq__(self, other):
        if isinstance(other, WeightedUndirectedGraph): return (self.node_weights(), self.link_weights()) == (other.node_weights(), other.link_weights())
        return False
