...
class Dict:
    ...
    def __contains__(self, item):
        return item in [p[0] for p in self.__items]
...
class SortedKeysDict(Dict):
    ...
    def pop(self, item):
        ...
        while low < high:
            ...
            if self.__f(item) < self.__f(self.__items[mid][0]):
                ...
                low = mid
    def copy(self):
        return SortedKeysDict(*self.__items, self.__f)
    def __len__(self):
        return len(self.__items)
    def __contains__(self, item):
        low, high = 0, len(self)
    ...
    def __eq__(self, other):
        if isinstance(other, SortedKeysDict):
            for p in self.__items:
                if p not in other.__items:
                    return False
            return len(self) == len(other)
        return False
    def __str__(self):
        return '{' + ', '.join(f'({k} -> {self.__f(k)}): {v}' for k, v in self.__items) + '}'
class Node:
