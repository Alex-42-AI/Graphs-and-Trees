...
from Personal.Graphs.src.general import *

from Personal.Graphs.src.tree import Tree, WeightedTree
...
class UndirectedGraph(Graph):
    def __init__(self, neighborhood: dict[Node, Iterable[Node]] = {}) -> None:
        """
        Create an unweighted undirected graph via adjacency list.
        """
    ...
    def add(self, u: Node, *current_nodes: Node) -> "UndirectedGraph":
        """
        Add a new node to given already present nodes.
        """
        if u not in self:
            self.__nodes.add(u)
