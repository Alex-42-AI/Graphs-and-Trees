...
class UndirectedGraph(Graph):
    ...
    def weighted_links_graph(self, weights: dict[Link, float] = None) -> "WeightedLinksUndirectedGraph":
        ...
        for l in self.links - set(weights):
            weights[l] = 0
        return WeightedLinksUndirectedGraph({u: {v: weights[Link(u, v)] for v in self.neighbors(u)} for u in self.nodes})

    def weighted_graph(self, node_weights: dict[Node, float] = None, link_weights: dict[Link, float] = None) -> "WeightedUndirectedGraph":
