        ...
        for i in range(len(descendants)):
            for j in range(i + 1, len(descendants)):
                if descendants[i] == descendants[j]:
                    raise ValueError('Can\'t have a node twice in a tree!')
        self.__hierarchy, self.__nodes, self.__links, self.__leaves = SortedKeysDict((self.__root, SortedList(f)), f=self.__f) + SortedKeysDict(*[(n, SortedList(f)) for n in descendants], f=self.__f), SortedList(f), [(root, n) for n in descendants], SortedList(f)
