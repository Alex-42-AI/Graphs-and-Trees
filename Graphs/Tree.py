...
class BinTree:
    def __init__(self, root=None): self.root = root if isinstance(root, BinNode) else BinNode(root)
    ...
    def left(self): return BinTree(self.root.left)
    def right(self): return BinTree(self.root.right)
    ...
    def encode(self, message: str):
        ...
        for c in message.upper():
            if c in self: res += self.code_in_morse(c) + '   '
            else: res += c + '  '
        return res[:-2]
    def invert(self):
        def dfs(node):
        ...
        dfs(self.root)
    def __invert__(self): self.invert()
    ...
    def print(self, traversal_type: str = 'in-order'):
        if traversal_type.lower() == 'preorder': print(self.__preorder_print())
        elif traversal_type.lower() == 'in-order': print(self.__in_order_print())
        elif traversal_type.lower() == 'post-order': print(self.__post_order_print())
        else: print('Traversal type ' + str(traversal_type) + ' is not supported!')
    def __str__(self): return str(self.__in_order_print())
...
class Tree:
    def __init__(self, root: Node, *descendants: Node):
        ...
        else: self.__leaves.insert(root)
    def root(self): return self.__root
    def nodes(self): return self.__nodes
    def links(self): return self.__links
    def leaves(self): return self.__leaves
    def hierarchy(self, u: Node = None): return self.__hierarchy if u is None else self.__hierarchy[u]
    def descendants(self, n: Node): return self.hierarchy(n)
    ...
    def dominating_set(self):
        dp = SortedKeysDict(*[(n, [1, 0]) for n in self.nodes().value()])
        def dfs(r):
            if not self.descendants(r): return
            s0, s1, s2 = 0, 0, 0
    ...
    def isomorphic(self, other):
        if isinstance(other, Tree):
            if len(self.nodes()) != len(other.nodes()) or len(self.links()) != len(other.links()) or len(self.leaves()) != len(other.leaves()) or len(self.descendants(self.root())) != len(other.descendants(other.root())): return False
            this_hierarchies, other_hierarchies = dict(), dict()
            for n in self.nodes().value():
                descendants = len(self.descendants(n))
                if descendants not in this_hierarchies: this_hierarchies[descendants] = 1
                else: this_hierarchies[descendants] += 1
            for n in other.nodes().value():
                descendants = len(other.descendants(n))
                if descendants not in other_hierarchies: other_hierarchies[descendants] = 1
                else: other_hierarchies[descendants] += 1
...
class WeightedNodesTree(Tree):
    ...
    def weights(self, u: Node = None): return self.__weights if u is None else self.__weights[u]
    def copy(self):
        ...
        if isinstance(tree, WeightedNodesTree):
            ...
            while queue:
                ...
                if res:
                    ...
                    queue += res
    def remove(self, u: Node): self.__weights.pop(u), super().remove(u)
    def vertex_cover(self):
