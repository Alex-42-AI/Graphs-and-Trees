...
class Tree:
    ...
    def hierarchy(self, u: Node = None):
        return SortedKeysDict(*filter(lambda p: p[1], self.__hierarchy.items())) if u is None else self.__hierarchy[u]
    ...
    def add_nodes_to(self, curr: Node, u: Node, *rest: Node):
        ...
        for v in [u] + [*rest]:
            if v not in self.nodes():
                ...
                self.__hierarchy[v] = []
    def copy(self):
        res = Tree(self.root(), *self.descendants(self.root()))
...
class WeightedNodesTree(Tree):
        ...
        while queue:
            ...
            if res_descendants:
                ...
                for d in res_descendants:
                    res.__weights[d] = self.weights(d)
                queue += res_descendants
        return res
    ...
    def extend_tree_at(self, n: Node, tree):
        ...
        if isinstance(tree, WeightedNodesTree):
            ...
            while queue:
                u = queue.pop(0)
                res = list(filter(lambda _n: _n not in self.nodes(), tree.descendants(u)))
                if res:
    ...
    def vertex_cover(self):
        dp = SortedKeysDict(*[(n, [self.weights(n), 0]) for n in self.nodes()])
        def dfs(u: Node):
    ...
    def dominating_set(self):
        ...
        def dfs(r):
            ...
            for d in self.descendants(r):
                dfs(d)
                s0 += dp[d][1]
                s1, s2 = s1 + dp[d][0], min(s2, self.weights(d) + sum(dp[w][1] for w in self.descendants(d)) + sum(dp[_d][0] for _d in self.descendants(r)) - dp[d][0])
            dp[r][0] += min(s0, s2)
