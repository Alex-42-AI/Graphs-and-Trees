...
class DirectedGraph:
    ...
    def add(self, u: Node, pointed_by: [Node] = None, points_to: [Node] = None):
        if u not in self.nodes():
            ...
            self.__degrees[u], self.__next[u], self.__prev[u] = [len(res_pointed_by), len(res_points_to)], SortedList(
                self.__f), SortedList(self.__f)
    ...
    def cut_nodes(self):
        ...
        levels, colors, res = SortedKeysDict(*[(n, 0) for n in self.nodes().value()], f=self.__f), SortedKeysDict(
            *[(n, 0) for n in self.nodes().value()], f=self.__f), []
        dfs(self.nodes()[0], 0)
        return res
    ...
    def hamiltonWalkExists(self, u: Node, v: Node):
        if u in self.next(v): return True if all(
    ...
    def __str__(self):
        return '({' + ', '.join(str(n) for n in self.nodes().value()) + '}, {' + ', '.join(
            str(l[0]) + '->' + str(l[1]) for l in self.links()) + '})'
...
class WeightedNodesDirectedGraph(DirectedGraph):
        ...
        def dfs(x, curr_path, curr_w, total_negative, res_path=None, res_w=0):
            ...
            for y in filter(lambda _y: (x, _y) not in curr_path, self.next(x).value()):
                ...
                if y == v and (curr_w + self.node_weights(y) < res_w or not res_path): res_path, res_w = curr_path + [
                    (x, y)], curr_w + self.node_weights(y)
    ...
    def __add__(self, other):
        ...
        if isinstance(other, DirectedGraph):
            ...
            return res
        raise TypeError(
            f"Addition not defined between class WeightedNodesDirectedGraph and type {type(other).__name__}!")
...
class WeightedLinksDirectedGraph(DirectedGraph):
    ...
    def minimalPathLinks(self, u: Node, v: Node):
        def dfs(x, curr_path, curr_w, total_negative, res_path=None, res_w=0):
            ...
            for y in filter(lambda _y: (x, _y) not in curr_path, self.next(x).value()):
                ...
                if y == v and (curr_w + self.link_weights(
                    (x, y)) < res_w or not res_path): res_path, res_w = curr_path.copy() + [
                    (x, y)], curr_w + self.link_weights((x, y))
                curr = dfs(y, curr_path + [(x, y)], curr_w + self.link_weights((x, y)),
                           total_negative - self.link_weights((x, y)) * (self.link_weights((x, y)) < 0), res_path,
                           res_w)
                if curr[1] < res_w or not res_path: res_path, res_w = curr
            return res_path, res_w
    ...
    def hamiltonTourExists(self):
        def dfs(x):
            ...
            for y in tmp1.value():
                res = dfs(y)
                if res:
                    self.add(x, [(t, tmp0weights[(t, x)]) for t in tmp0.value()],
                             [(t, tmp1weights[(x, t)]) for t in tmp1.value()])
                    return True
            self.add(x, [(t, tmp0weights[(t, x)]) for t in tmp0.value()],
                     [(t, tmp1weights[(x, t)]) for t in tmp1.value()])
            return False
    ...
    def isomorphic(self, other):
        if isinstance(other, WeightedLinksDirectedGraph):
            if len(self.links()) != len(other.links()) or len(self.nodes()) != len(other.nodes()): return False
            this_degrees, other_degrees, this_weights, other_weights = SortedKeysDict(f=self.__f), SortedKeysDict(
                f=self.__f), dict(), dict()
            for d in self.degrees().values():
                if d in this_degrees:
                    this_degrees[d] += 1
                else:
    ...
    def __add__(self, other):
        ...
        if isinstance(other, DirectedGraph):
            ...
            return res
        raise TypeError(
            f"Addition not defined between class WeightedLinksDirectedGraph and type {type(other).__name__}!")
...
class WeightedDirectedGraph(WeightedNodesDirectedGraph, WeightedLinksDirectedGraph):
    ...
    def remove(self, u: Node, *rest: Node):
        for n in (u,) + rest: self._WeightedNodesDirectedGraph__node_weights.pop(n)
    ...
    def minimalPath(self, u: Node, v: Node):
        def dfs(x, curr_path, curr_w, total_negative, res_path=None, res_w=0):
            ...
            for y in filter(lambda _y: (x, _y) not in curr_path, self.next(x).value()):
                if curr_w + self.link_weights(x, y) + total_negative >= res_w and res_path: continue
                if y == v and (curr_w + self.link_weights(x, y) + self.node_weights(
                    y) < res_w or not res_path): res_path, res_w = curr_path + [(x, y)], curr_w + self.link_weights(x,
                                                                                                                    y) + self.node_weights(
                    y)
                curr = dfs(y, curr_path + [(x, y)], curr_w + self.link_weights(x, y) + self.node_weights(y),
                           total_negative - self.link_weights(x, y) * (self.link_weights(x, y) < 0) - self.node_weights(
                               y) * (self.node_weights(y) < 0), res_path, res_w)
                if curr[1] < res_w or not res_path: res_path, res_w = curr
            return res_path, res_w
    ...
    def isomorphic(self, other):
        if isinstance(other, WeightedDirectedGraph):
            ...
            for possibility in possibilities:
                ...
                for n, u in map_dict:
                    for m, v in map_dict:
                        if self.link_weights(n, m) != other.link_weights(u, v) or self.node_weights(
    ...
    def __add__(self, other):
        ...
        if isinstance(other, WeightedDirectedGraph):
            ...
            raise TypeError(
                f"Addition not defined between class WeightedDirectedGraph and type {type(other).__name__}!")
        return res
