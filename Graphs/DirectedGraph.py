...
class DirectedGraph:
    ...
    def add(self, u: Node, pointed_by: [Node] = None, points_to: [Node] = None):
        if u not in self.nodes():
            ...
            self.__degrees[u], self.__next[u], self.__prev[u] = [len(res_pointed_by), len(res_points_to)], SortedList(self.__f), SortedList(self.__f)
    ...
    def copy(self):
        ...
        for u in self.nodes().value():
            if self.degrees(u)[1]:
                res.connect_from_to(u, *self.next(u).value())
        return res
    ...
    def connected(self):
        ...
        while queue:
            u = queue.pop(0)
            for v in filter(lambda x: x not in total, self.prev(u).value() + self.next(u).value()):
    ...
    def sources(self):
        return [u for u in self.nodes().value() if not self.degrees(u)[0]]

    def sinks(self):
        return [v for v in self.nodes().value() if not self.degrees(v)[1]]
    ...
    def toposort(self):
        if not self.dag():
            return []
        queue, res, total = self.sources(), [], SortedList(self.__f)
        while queue:
            u = queue.pop(0)
            res.append(u), total.insert(u)
            for v in filter(lambda x: x not in total, self.next(u).value()):
    ...
    def cut_nodes(self):
        ...
        levels, colors, res = SortedKeysDict(*[(n, 0) for n in self.nodes().value()], f=self.__f), SortedKeysDict(*[(n, 0) for n in self.nodes().value()], f=self.__f), []
        dfs(self.nodes()[0], 0)
        return res
            ...
            for y in self.next(x).value():
                ...
                if y == n:
                    ...
                    while stack and curr_node != n:
                        ...
                        curr_node = stack.pop()
                    for curr_node in curr.value():
                        helper(curr_node, [curr_node])
                    return

        total, res = SortedList(self.__f), []
        for n in self.nodes().value():
            if n not in total:
                curr = SortedList(self.__f)
    ...
    def hamiltonTourExists(self):
        def dfs(x):
            ...
            if all(y not in self.nodes() for y in can_end_in.value()):
                return False
    ...
    def isomorphic(self, other):
        if isinstance(other, DirectedGraph):
            if len(self.links()) != len(other.links()) or len(self.nodes()) != len(other.nodes()):
                return False
            this_degrees, other_degrees = SortedKeysDict(f=self.__f), SortedKeysDict(f=self.__f)
            for d in self.degrees().values():
            ...
            if this_degrees != other_degrees:
                return False
            this_nodes_degrees, other_nodes_degrees = SortedKeysDict(*[(d, []) for d in this_degrees.keys()], f=self.__f), SortedKeysDict(*[(d, []) for d in other_degrees.keys()], f=self.__f)
    ...
    def __str__(self):
        return '({' + ', '.join(str(n) for n in self.nodes().value()) + '}, {' + ', '.join(str(l[0]) + '->' + str(l[1]) for l in self.links()) + '})'
...
class WeightedNodesDirectedGraph(DirectedGraph):
    ...
    def copy(self):
        ...
        for n in self.nodes().value():
            if self.degrees(n)[1]:
                res.connect_from_to(n, *self.next(n).value())
        return res
    ...
    def minimalPathNodes(self, u: Node, v: Node):
        def dfs(x, curr_path, curr_w, total_negative, res_path=None, res_w=0):
            ...
            for y in filter(lambda _y: (x, _y) not in curr_path, self.next(x).value()):
                ...
                curr = dfs(y, curr_path + [(x, y)], curr_w + self.node_weights(y),
...
class WeightedLinksDirectedGraph(DirectedGraph):
    ...
    def link_weights(self, u_or_l: Node | tuple = None, v: Node = None):
        if u_or_l is None:
            ...
            if v is None:
                return SortedKeysDict(*[(n, self.__link_weights[(u_or_l, n)]) for n in self.next(u_or_l).value()], f=self._DirectedGraph__f)
    ...
    def hamiltonTourExists(self):
        def dfs(x):
            ...
            if all(y not in self.nodes() for y in can_end_in.value()):
                return False
            tmp0, tmp1 = self.prev(x), self.next(x)
            tmp0weights, tmp1weights = SortedKeysDict(*[(t, self.link_weights(t, x)) for t in tmp0.value()], f=self._DirectedGraph__f), SortedKeysDict(*[(t, self.link_weights(x, t)) for t in tmp1.value()], f=self._DirectedGraph__f)
            self.remove(x)
            for y in tmp1.value():
                res = dfs(y)
                if res:
    ...
    def hamiltonWalk(self, u: Node = None, v: Node = None):
        def dfs(x, stack):
            ...
            if not self.nodes():
                return stack
            tmp0, tmp1 = self.prev(x), self.next(x)
            tmp0weights, tmp1weights = SortedKeysDict(*[(t, self.link_weights(t, x)) for t in tmp0.value()], f=self.__f), SortedKeysDict(*[(t, self.link_weights(x, t)) for t in tmp1.value()], f=self.__f)
            self.remove(x)
            for y in tmp1.value():
                if y == v:
                    if [*self.nodes().value()] == [v]:
    ...
    def isomorphic(self, other):
        if isinstance(other, WeightedLinksDirectedGraph):
            if len(self.links()) != len(other.links()) or len(self.nodes()) != len(other.nodes()):
                return False
            this_degrees, other_degrees, this_weights, other_weights = SortedKeysDict(f=self.__f), SortedKeysDict(f=self.__f), dict(), dict()
            for d in self.degrees().values():
                if d in this_degrees:
                    this_degrees[d] += 1
                else:
    ...
    def __add__(self, other):
        if isinstance(other, (WeightedDirectedGraph, WeightedNodesDirectedGraph)):
            return other + self
        res = self.copy()
        if isinstance(other, WeightedLinksDirectedGraph):
            for n in other.nodes().value():
    ...
    def __str__(self):
        return '({' + ', '.join(str(n) for n in self.nodes().value()) + '}, ' + f'{self.link_weights()}' + ')'


class WeightedDirectedGraph(WeightedNodesDirectedGraph, WeightedLinksDirectedGraph):
    def __init__(self, *pairs: (Node, float), f=lambda x: x):
        WeightedNodesDirectedGraph.__init__(self, *pairs, f=f)
    ...
    def add(self, n_w: (Node, float), *nodes_weights: (Node, float)):
        if n_w[0] not in self.nodes():
            self._WeightedNodesDirectedGraph__node_weights[n_w[0]] = n_w[1]
    ...
    def minimalPath(self, u: Node, v: Node):
        ...
        if u in self.nodes() and v in self.nodes():
            if self.reachable(u, v):
                return dfs(u, [], self.node_weights(u), sum(self.link_weights(l) for l in self.links() if self.link_weights(l) < 0) + sum(self.node_weights(n) for n in self.nodes().value() if self.node_weights(n) < 0))
            return [], 0
        raise ValueError('Unrecognized node(s)!')
    ...
    def isomorphic(self, other):
        if isinstance(other, WeightedDirectedGraph):
            if len(self.links()) != len(other.links()) or len(self.nodes()) != len(other.nodes()):
            return False
            this_degrees, other_degrees, this_node_weights, other_node_weights, this_link_weights, other_link_weights = SortedKeysDict(f=self._DirectedGraph__f), SortedKeysDict(f=self._DirectedGraph__f), dict(), dict(), dict(), dict()
            for d in self.degrees().values():
                if d in this_degrees:
                    this_degrees[d] += 1
                else:
            ...
            if this_degrees != other_degrees or this_node_weights != other_node_weights or this_link_weights != other_link_weights:
                return False
            this_nodes_degrees, other_nodes_degrees = SortedKeysDict(*[(d, []) for d in this_degrees.keys()], f=self._DirectedGraph__f), SortedKeysDict(*[(d, []) for d in other_degrees.keys()], f=self._DirectedGraph__f)
            for d in this_degrees.keys():
                for n in self.nodes().value():
                    if self.degrees(n) == d:
                        this_nodes_degrees[d].append(n)
            ...
            return False
        if isinstance(other, WeightedNodesDirectedGraph):
            return WeightedNodesDirectedGraph.isomorphic(self, other)
        if isinstance(other, WeightedLinksDirectedGraph):
            return WeightedLinksDirectedGraph.isomorphic(self, other)
    ...
    def __add__(self, other):
        ...
        if isinstance(other, WeightedDirectedGraph):
            ...
            raise TypeError(f"Addition not defined between class WeightedDirectedGraph and type {type(other).__name__}!")
        return res
